...
class ClosingEdge(override var from: Node,
    ...
    override fun timeIntervalForward(values: List<Double>): List<Double> {
        return listOf(values[from.minArity], values[from.minArity+1])
    }
    ...
    override fun copy(): Edge {
        return ClosingEdge(from=from, to=to, isCyclic=isCyclic, termOrder=termOrder, t1=t1, t2=t2, termOrderShuffleAllowed = termOrderShuffleAllowed, termOrderReference = termOrderReference)
    }

    override fun equals(other: Any?): Boolean {
        if (this === other) return true
        if (other !is ClosingEdge) return false
    ...
    override fun toString(): String {
        return "ClosingEdge(from=$from, to=$to, isCyclic=$isCyclic, termOrder=$termOrder, t1=$t1, t2=$t2, uniqueId='$uniqueId')"
    }
