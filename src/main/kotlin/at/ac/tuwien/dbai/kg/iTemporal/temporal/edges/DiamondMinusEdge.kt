...
class DiamondMinusEdge(override var from: Node,
    ...
    override fun timeIntervalBackward(values: List<Double>): List<Double> {
        return listOf(values[to.minArity] - t1, values[to.minArity+1] - t2)
    }

    override fun timeIntervalForward(values: List<Double>): List<Double> {
        return listOf(values[from.minArity] + t1, values[from.minArity+1] + t2)
    }

    override fun applyInterval(interval: Interval): Interval {
        return Interval(interval.t1 + this.t1, interval.t2 + this.t2)
    }
    ...
    override fun copy(): Edge {
        return DiamondMinusEdge(from=from, to=to, isCyclic=isCyclic, termOrder=termOrder, t1=t1, t2=t2, termOrderShuffleAllowed = termOrderShuffleAllowed, termOrderReference = termOrderReference)
    }

    override fun equals(other: Any?): Boolean {
        if (this === other) return true
        if (other !is DiamondMinusEdge) return false
    ...
    override fun toString(): String {
        return "DiamondMinusEdge(from=$from, to=$to, isCyclic=$isCyclic, termOrder=$termOrder, t1=$t1, t2=$t2, uniqueId='$uniqueId')"
    }
